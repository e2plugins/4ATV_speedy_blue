...
class speedy_ServiceInfoEX(Poll, Converter, object):
    apid = 0
    vpid = 1
    sid = 2
    onid = 3
    ...
    def getText(self):
        ...
        if self.getServiceInfoString(info, iServiceInformation.sFrameRate, lambda x: "%d" % ((x+500)/1000)) != "N/A":
            self.stream['fps'] = self.getServiceInfoString(info, iServiceInformation.sFrameRate, lambda x: "%d Hz" % ((x+500)/1000))
