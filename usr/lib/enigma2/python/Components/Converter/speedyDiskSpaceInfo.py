# Embedded file name: /usr/lib/enigma2/python/Components/Converter/blueDiskSpaceInfo.py
...
class speedyDiskSpaceInfo(Poll, Converter):
    HDDTEMP = 0
    LOADAVG = 1
    MEMTOTAL = 2
    MEMFREE = 3
    ...
    def getText(self):
        ...
        if self.type == self.HDDTEMP:
            ...
            if list[0] == 0:
                ...
                text = ('%s: %s ' + _('Free: ') + '%s ' + _('Used:') + ' %s %s%%') % (entry[1],
                 self.getSizeStr(list[0]),
                 self.getSizeStr(list[2]),
                 self.getSizeStr(list[1]),
                 list[3])
            else:
                text = ('%s: %s ' + _('Used:') + ' %s ' + _('Free: ') + '%s') % (entry[1],
                 self.getSizeStr(list[0]),
                 self.getSizeStr(list[1]),
                 self.getSizeStr(list[2]))
        return text
