...
class speedyInfo2(Poll, Converter, object):
    CAID = 0
    PID = 1
    PROV = 2
    ALL = 3
    ...
    internet = "internettime"
    cardserver = "cardservertime"
    ...
    def __init__(self, type):
        ...
        elif type == "Cardserver":
            self.type = self.CARDSERVER
                ...
                if info.getInfoObject(iServiceInformation.sCAIDs):
                    ...
                    if ecm_info:
                        ...
                        if self.type == self.FORMAT:
                            ...
                            return textvalue[:-1]
                            ########################################################
                            if self.type == self.INTERNET:
                                if caid == "internet":
                                    return True
                                return False

                            if self.type == self.cardserver:
                                if caid == "cardserver":
                                    return True
                                return False
                            ########################################################
                        ...
                        if self.type == self.SHORT:
                            if source == "emu":
                                ...
                                textvalue = "%s - Prov: %s, Caid: %s - %s" % (source, prov, caid, ecm_time.replace('msec','mms'))
                            else:
                                try:
                                #############################  meine ####################################
                                    textvalue = "Reader: %s  Server:  %s  Caid: %s  Hops: %s  ECM: %s" % ( reader, server, caid, hops, ecm_time.replace('msec','ms'))
                                except:
                                    pass
                                    #############################  secound infobar ####################################
                        if self.type == self.TEST:
                            if source == "emu":
                                ...
                                try:
                                #############################  meine ####################################
                                    textvalue = "ECM:  %s" % ( ecm_time.replace('msec','ms'))
                                except:
                                    pass
                    else:
                        if self.type == self.ALL or self.type == self.SHORT or self.type == self.TEST or (self.type == self.FORMAT and (self.sfmt.count("%") > 3 )):
