...
class MSNWeather3(Poll, Converter, object):
...
# Орбита меркурия
        ...
        SPR = math.fmod((RA - BETA - STT * 15) / 15 * 0.997269566423530, 24)
        if SPR < 0:
            SPR = SPR + 24
        SPS = math.fmod((RA + BETA - STT * 15) / 15 * 0.997269566423530, 24)
        if SPS < 0:
            SPS = SPS + 24
        if SPR < SPS:
            SPC = (SPR + SPS) / 2
...
#
        ...
        try:
            ...
            msnweather['Moonrise'] = '%s%s%s%s' % (str(MRh).zfill(2), six.unichr(58).encode("latin-1"), MRx, MRm)
